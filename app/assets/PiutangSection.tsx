...
import { Plus, Trash2, ChevronDown, Check, X, Clock, Handshake } from 'lucide-react'
...
export default function PiutangSection({ initialData, persons, openAddForm, onAddFormClose }: Props) {
  ...
  // Sync external openAddForm prop
  useEffect(() => {
    if (openAddForm) {
      setShowAddForm(true)
    }
  ...
  return (
    <div>
              ...
              <div key={p.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center flex-shrink-0">
                    <span className="text-lg"><Handshake size={20} /></span>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 dark:border-gray-700">
                    ...
                    {addingPaymentFor === p.id ? (
                      <BottomDrawer open={true} onClose={() => setAddingPaymentFor(null)} title="Catat Pembayaran">
                        <form
                        ...
                        >
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="w-full pl-10 pr-3 py-3 border border-gray-200 dark:border-gray-700 rounded-xl text-lg font-semibold bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-base-500"
                                autoFocus
                              />
                            </div>
                          </div>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 btn-base h-[48px] rounded-xl font-semibold text-sm"
                            >
                              Catat Bayar
                            </button>
                            <button
                              ...
                              className="flex-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 h-[48px] rounded-xl font-semibold text-sm border border-gray-200 dark:border-gray-700"
                            >
                              Batal
                            </button>
                          </div>
      ...
      {/* Add Piutang Drawer */}
      <BottomDrawer open={showAddForm} onClose={() => { setShowAddForm(false); onAddFormClose?.() }} title="Tambah Piutang">
        <form onSubmit={handleAdd} className="space-y-4">
          <div>
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 block">Nama Peminjam</label>
            <input
              type="text"
              value={newDebtorName}
          ...
          >
            ...
            <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
              newDeductFromBalance ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300 dark:border-gray-600'
            }`}>
              {newDeductFromBalance && (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {persons.map((p) => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => setNewPersonId(p.id)}
                      className={`flex-1 min-w-[80px] py-2 rounded-xl text-sm font-medium border transition-colors ${
                        newPersonId === p.id ? colors.button : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-gray-200 dark:border-gray-700'
                      }`}>
                      {p.name}
                    </button>
                  )
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={isPending || !newDebtorName || !newAmount} className="flex-1 btn-base h-[48px] rounded-xl font-semibold text-sm">
              Tambah
            </button>
            <button type="button" onClick={() => { setShowAddForm(false); onAddFormClose?.() }} className="flex-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 h-[48px] rounded-xl font-semibold text-sm border border-gray-200 dark:border-gray-700">
              Batal
            </button>
          </div>
        </form>
